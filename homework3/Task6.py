"""
Реализовать функцию int_func(), принимающую слово из маленьких латинских букв и
возвращающую его же, но с прописной первой буквой. Например, print(int_func(‘text’)) -> Text.
Продолжить работу над заданием. В программу должна попадать строка из слов, разделенных
пробелом. Каждое слово состоит из латинских букв в нижнем регистре. Сделать вывод исходной
строки, но каждое слово должно начинаться с заглавной буквы. Необходимо использовать
написанную ранее функцию int_func().
"""


def get_capitalize_string(input_string: str) -> str:
    result = str.capitalize(input_string)
    return result


while True:
    my_string = input('Введите слова через пробел:\n')
    if my_string.isascii():
        my_result = list(map(get_capitalize_string, my_string.split()))
        print(' '.join(my_result))
        break
    elif my_string.lower() == 'q':
        break
    else:
        print('Latin symbols only')













